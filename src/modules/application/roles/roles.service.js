...
export default class RolesService {
  ...
  async getAllRoles(filters) {
    const { page = 1, limit = 10, status, searchTerm, dateRange, sort } = filters;
  ...
  async getRoleById(id) {
    return await Role.findById(id).populate('createdBy', 'firstName lastName email').lean();
  }
  ...
  async getRoleByName(name) {
    return await Role.findOne({ name }).populate('createdBy', 'firstName lastName email').lean();
  }
